...
md"""
The Landau-Zener Hamiltonian is defined as:
"""
...
md"""
We use a time grid that goes from ``-\infty`` to ``+\infty``. Numerically, that is from ``-T`` to ``+T``, where we can set ``T = `` $(@bind Γ Scrubbable(2:100; default=10)) ``/ α``
"""
...
md"""
The resulting 2×2 Hamiltonian at ``t =`` $(slider_t = @bind t Slider(range(-T, T; length=1001); default=0, show_value=true)) is
"""
...
md"""
with an eigen-decomposition of
"""

# ╔═╡ 83347e94-bc94-40ac-a518-cb60909afe2f
eigen(Array(evaluate(H, t)))

# ╔═╡ 509aa2c6-94be-4fd4-b318-66b98099d11b
md"""
Instead of looking at a particular point in time, we can make plots for different values of `t`: The eigenvalues, and for each of the two instantaneous eigenstates, the projection onto the canonical basis:
"""
...
begin
	function show_eigen_plot()
		...
		eigstate1_1 = [abs2(es.vectors[:,2] ⋅ ϕ₁) for es in eigensys]
		ax1 = plot(tlist, eigvals0, label="0")
		plot!(ax1, tlist, eigvals1, label="1")
		plot!(ax1;xlabel="time", ylabel="energy", legend=:right, title="eigenvalues")
		ax2 = plot(tlist, eigstate0_0, label="|⟨Ψ₀|0⟩|²")
		plot!(ax2, tlist, eigstate0_1, label="|⟨Ψ₀|1⟩|²")
		plot!(ax2;xlabel="time", ylabel="population", legend=:right, title="eigenstate 0")
		ax3 = plot(tlist, eigstate1_0, label="|⟨Ψ₁|0⟩|²")
		plot!(ax3, tlist, eigstate1_1, label="|⟨Ψ₁|1⟩|²")
		plot!(ax3;xlabel="time", ylabel="population", legend=:right, title="eigenstate 1")
		plot(ax1, ax2, ax3)
	end
	show_eigen_plot()
end

# ╔═╡ 6d12ea1c-c51b-4734-a706-3226a4cef2fd
md"""
* ``\Delta =`` $(select_Δ)
* ``\alpha =`` $(select_α)
"""

# ╔═╡ 7b669cbe-187c-4830-8363-1b88db0dd60b
md"""
Now we can look at the *dynamic* behavior of a state evolution over a given time grid:
"""

# ╔═╡ 2e243c2f-3d13-4eb4-9005-c821b6c0512c
...
begin
	...
	function QuantumPropagators.Storage.map_observable(
		...
		H = Array(evaluate(observable.H, tlist[i]))
		Ψ₀ = eigen(H).vectors[:,1]
		Ψ₁ = eigen(H).vectors[:,2]
		return [abs2(state ⋅ Ψ₀), abs2(state ⋅ Ψ₁)]
	end
end

# ╔═╡ 45d3f8e7-455e-467e-9c6b-bf26496a4cec
md"""
With the propagation method
"""

# ╔═╡ c10991a2-8722-4a5b-8c7f-79b4a8229ef8
@bind prop_method Select([ExpProp, OrdinaryDiffEq])

# ╔═╡ 98cf7d00-010d-43c3-a750-af010c749903
md"""
we get the following dynamics:
"""
...
begin
	function show_dynamics()
		pop_dyn = propagate(
			...
			method=prop_method,
			storage=true,
			observables=(Ψ->abs2.(Ψ), DynamicEigenstate(H)),
			#observables=(Ψ->abs2.(Ψ), ),
			check=false,
			reltol=0.0, abstol=1e-15
		)
		pop_canonical = reduce(hcat, pop_dyn[1,:])
		pop_instantaneous = reduce(hcat, pop_dyn[2,:])
		ax1 = plot(
			tlist, pop_canonical';
			label=["|⟨Ψ(t)|0⟩|²" "|⟨Ψ(t)|1⟩|²"],
			legend=:left,
...
manifest_format = "2.0"
project_hash = "350fdae702432d252c5baa482aa721cf002c5070"
...
[[deps.QuantumControl]]
deps = ["FileIO", "IOCapture", "JLD2", "LinearAlgebra", "Logging", "Pkg", "Printf", "QuantumPropagators", "UUIDs"]
git-tree-sha1 = "bf2b978e6a3fcba1a01e741411e0d389c6617c64"
uuid = "8a270532-f23f-47a8-83a9-b33d10cad486"
version = "0.11.1"

    [deps.QuantumControl.extensions]
    QuantumControlFiniteDifferencesExt = "FiniteDifferences"
    QuantumControlZygoteExt = "Zygote"

    [deps.QuantumControl.weakdeps]
    FiniteDifferences = "26cc04aa-876d-5657-8c51-4c34ba976000"
    Zygote = "e88e6eb3-aa80-5325-afca-941959d7151f"

[[deps.QuantumPropagators]]
deps = ["LinearAlgebra", "OffsetArrays", "Printf", "ProgressMeter", "Random", "SparseArrays", "SpecialFunctions", "Test", "TimerOutputs"]
git-tree-sha1 = "6a2d8875b22f74862d41cf5ce2b804ea3b333ba9"
uuid = "7bf12567-5742-4b91-a078-644e72a65fc1"
version = "0.8.3"
weakdeps = ["OrdinaryDiffEq", "RecursiveArrayTools", "StaticArrays"]

    [deps.QuantumPropagators.extensions]
...
git-tree-sha1 = "f9195449e0ae7e8daf9d609c9619ecfc2369f62c"
